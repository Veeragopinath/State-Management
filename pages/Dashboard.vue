<template>
  <v-card class="mainPage outlined mx-3 px-4">
    <v-row class="px-3 py-2">
      ...
      <v-spacer> </v-spacer>
      <v-btn  :disabled="isDisabled" class="blue white--text px-3 py-1" @click="userAddMode = true"
    ...
    <v-sheet outlined class="mt-1">
      ...
      >
        ...
        <template #[`item.actions`]="{ item }">
          <div class="d-flex">
            <v-btn :disabled="isDisabled" class="crudButtonPrimary px-3 py-0 mr-2" @click="editUser(item)">
              Edit
            </v-btn>
            <v-btn :disabled="isDisabled" color="grey" class="px-3 py-0" @click="deleteUserDialog(item)">
              Delete
            </v-btn>
          </div>
